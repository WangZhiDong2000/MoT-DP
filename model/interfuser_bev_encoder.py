...
class InterfuserBEVEncoder(nn.Module):
    ...
    设计理念:
        - 不再使用低维状态引导的BEV特征池化
        - BEV tokens作为独立特征传递给decoder-only架构
        - 保留完整的空间信息用于sequential cross-attention
        - state信息由transformer直接处理ego_status历史序列
    """
    ...
    def forward_batch(
        ...
        """
        Extract BEV tokens for a batch with temporal dimension.
        ...
        if use_precomputed:
            ...
            bev_tokens_flat = self.forward(
                lidar_token=lidar_tokens_flat,
                lidar_token_global=lidar_tokens_global_flat
            )
        else:
            if images is None:
                raise ValueError("Either images or (lidar_tokens, lidar_tokens_global) must be provided")
            
            B, T = images.shape[:2]
            images_flat = images.reshape(B * T, *images.shape[2:])
            
            bev_tokens_flat = self.forward(images_flat)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        bev_batch = encoder.forward_batch(images)
