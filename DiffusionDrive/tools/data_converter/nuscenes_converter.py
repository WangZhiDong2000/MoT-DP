...
def create_nuscenes_infos(root_path,
    ...
    # Try to initialize map extractor, but allow it to fail for datasets without map expansion files
    try:
        nusc_map_extractor = NuscMapExtractor(root_path, roi_size)
        print("Map extractor initialized successfully")
    except FileNotFoundError as e:
        print(f"Warning: Map expansion files not found: {e}")
        print("Continuing without map extraction...")
...
def _fill_trainval_infos(nusc,
    ...
    for sample in tqdm(nusc.sample, desc="Processing samples"):
        ...
        assert os.path.exists(lidar_path), f"Lidar file does not exist: {lidar_path}"
        ...
        # extract map annos (only if map extractor is available)
